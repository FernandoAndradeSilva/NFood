  ...
  public get(url: string): Promise<HttpResultModel> {
    ...
    return new Promise((resolve) => {
      ...
      if (this.networkSrv.IsOnline) {
        ...
        this.alertSrv.toast('Você está Offline, e infelizmente não podem ser carregado os dados!', 'bottom');
  ...
  public post(url: string, model: any): Promise<HttpResultModel> {
    ...
    return new Promise((resolve) => {
      if (this.networkSrv.IsOnline) {
        ...
        this.alertSrv.toast('Você está Offline, e infelizmente não podem ser enviados os dados!', 'bottom');
        resolve({ success: false, data: [], err: undefined });
      }
    })
  }
